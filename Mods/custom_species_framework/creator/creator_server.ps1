...
function Read-TextFile {
  ...
  try {
    return [string]::Concat((Get-Content -LiteralPath $Path -Raw -Encoding UTF8))
  } catch {
    return $Fallback
  }
}
...
function Update-CatalogCache {
  ...
  $raw = ""
  try {
    $raw = Get-Content -LiteralPath $script:CatalogFile -Raw -Encoding UTF8 -ErrorAction Stop
  } catch {
    $raw = ""
  }
...
function Update-CatalogSummaryCache {
  ...
  $raw = ""
  try {
    $raw = Get-Content -LiteralPath $script:CatalogSummaryFile -Raw -Encoding UTF8 -ErrorAction Stop
  } catch {
    $raw = ""
  }
...
function Convert-CatalogSpeciesToSummary {
  ...
  return @{
    ...
    variant_scope = $Entry["variant_scope"]
...
function Get-CatalogSpeciesPayload {
  ...
  try {
    return Convert-ToHashtable (ConvertFrom-Json (Get-Content -LiteralPath $detailFile -Raw -Encoding UTF8 -ErrorAction Stop))
  } catch {
    return @{
      ok = $false
...
function Build-SpeciesEntry {
  ...
  if ($kindValue -eq "regional_variant") {
    ...
    $variantScope = Normalize-DisplayText $Payload["variant_scope"]
    if (-not $variantScope) {
      $variantScope = "single_species"
    }
    $entry["variant_scope"] = $variantScope
...
function Add-MatchingVisualVariants {
  ...
  Get-ChildItem -LiteralPath $Directory -Filter $Filter -File -ErrorAction SilentlyContinue |
    Sort-Object Name |
    ForEach-Object {
      if ([string]::IsNullOrWhiteSpace($MatchPattern) -or $_.Name -match $MatchPattern) {
...
function Add-FusionSlicePreviewEntry {
  param(
    [hashtable]$EntriesByBody,
    [int]$HeadDex,
    [int]$BodyDex,
    [string]$AbsolutePath,
    [string]$Source
  )
  if ($HeadDex -le 0 -or $BodyDex -le 0) {
    return
  }
  if ([string]::IsNullOrWhiteSpace($AbsolutePath) -or -not (Test-Path -LiteralPath $AbsolutePath)) {
    return
  ...
  $key = [string]$BodyDex
  $entry = $EntriesByBody[$key]
  if ($null -eq $entry) {
    $entry = @{
      head_dex = $HeadDex
      body_dex = $BodyDex
      fusion_id = ("B{0}H{1}" -f $BodyDex, $HeadDex)
      preview_path = $publicPath
      variant_count = 0
      sources = @{}
      preview_paths = @{}
    }
    $EntriesByBody[$key] = $entry
  }

  if (-not $entry["preview_paths"].ContainsKey($publicPath)) {
    $entry["preview_paths"][$publicPath] = $true
    $entry["variant_count"] = [int]$entry["variant_count"] + 1
    if ([string]::IsNullOrWhiteSpace([string]$entry["preview_path"])) {
      $entry["preview_path"] = $publicPath
    }
  }
  if (-not [string]::IsNullOrWhiteSpace($Source) -and -not $entry["sources"].ContainsKey($Source)) {
    $entry["sources"][$Source] = $true
  }
}

function Get-FusionSlicePayload {
  param([System.Net.HttpListenerRequest]$Request)

  $headDex = Normalize-Int $Request.QueryString["head_dex"] 0 0
  $bodyDex = Normalize-Int $Request.QueryString["body_dex"] 0 0
  if ($headDex -le 0) {
    return @{ ok = $false; error = "Choose a fusion head first." }
  }

  $entriesByBody = @{}
  $pattern = "^{0}\.(?<body>\d+)(?:[A-Za-z]+|[_-][A-Za-z0-9]+)*\.png$" -f [regex]::Escape([string]$headDex)
  $scanTargets = @(
    @{ Directory = (Join-Path $script:GameRoot "Graphics\CustomBattlers\indexed\$headDex"); Source = "Custom Battler" },
    @{ Directory = (Join-Path $script:GameRoot "Graphics\Battlers\$headDex"); Source = "Battler" }
  )
  ...
  foreach ($target in $scanTargets) {
    ...
    Get-ChildItem -LiteralPath $directory -Filter *.png -File -ErrorAction SilentlyContinue |
      ...
      ForEach-Object {
        ...
        if ($match.Success) {
          $parsedBodyDex = Normalize-Int $match.Groups["body"].Value 0 0
          if ($parsedBodyDex -gt 0 -and ($bodyDex -le 0 -or $parsedBodyDex -eq $bodyDex)) {
            Add-FusionSlicePreviewEntry -EntriesByBody $entriesByBody -HeadDex $headDex -BodyDex $parsedBodyDex -AbsolutePath $_.FullName -Source ([string]$target["Source"])
          }
        }
      }
  }
  ...
  $entries = @(
    $entriesByBody.Values |
      Sort-Object { [int]$_["body_dex"] } |
      ForEach-Object {
        @{
          head_dex = [int]$_["head_dex"]
          body_dex = [int]$_["body_dex"]
          fusion_id = [string]$_["fusion_id"]
          preview_path = [string]$_["preview_path"]
          variant_count = [int]$_["variant_count"]
          sources = @($_["sources"].Keys | Sort-Object)
        }
      }
  )
...
function Handle-ApiRequest {
  ...
  switch ($path) {
    ...
    "/api/fusion-slice" {
      Write-JsonResponse -Response $Context.Response -StatusCode 200 -Body (Get-FusionSlicePayload -Request $Context.Request)
      return
    }
    "/api/state" {
