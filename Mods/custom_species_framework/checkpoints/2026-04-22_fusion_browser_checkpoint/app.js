...
function updateCatalogStatus(result) {
  ...
  if (result.loading) {
    ...
    pill.className = "status-pill";
    showMessage("Loading the installed game's Pokedex catalog in the background.", "info");
    return;
  }
  if (result.ok) {
  ...
  pill.className = "status-pill";
  showMessage(result.data.error || "The creator couldn't load its local Pokedex catalog views.", "error");
}
  ...
  return [
    ...
    },
    {
      ...
      summary: isRegionalVariant
        ...
        : "Give it front art and the Pokedex text that will sell it in-game.",
...
function renderDexSpeciesList() {
  ...
  if (speciesEntries.length === 0 && state.dexCatalogScope === "fusion") {
    list.innerHTML = `<p class="muted-copy">Pick a fusion head or body to build a fusion slice from the installed Pokédex.</p>`;
    return;
  }
...
function fusionBrowsePoolEntries() {
  return mergedDexEntries()
    .filter((entry) => Boolean(fusionComponentEntryById(entry.id)))
...
function fusionCatalogEntries() {
  const mergedEntries = mergedDexEntries();
  const pool = fusionBrowsePoolEntries();
  const signature = [
    state.fusionBuilder.headId,
    state.fusionBuilder.bodyId,
    state.dexSelectedId,
    pool.length
  ].join("\u0001");

  if (
    state.dexCache.fusionEntriesSource === mergedEntries &&
  ...
  const explicitHead = fusionComponentEntryById(state.fusionBuilder.headId);
  const explicitBody = fusionComponentEntryById(state.fusionBuilder.bodyId);
  let entries = [];

  if (explicitHead && explicitBody) {
    entries = [buildFusionSpeciesEntry(explicitHead, explicitBody)].filter(Boolean);
  } else if (explicitHead) {
    entries = pool.map((bodyEntry) => buildFusionSpeciesEntry(explicitHead, bodyEntry)).filter(Boolean);
  } else if (explicitBody) {
    entries = pool.map((headEntry) => buildFusionSpeciesEntry(headEntry, explicitBody)).filter(Boolean);
  } else if (selectedFusion) {
    entries = [buildFusionSpeciesEntryFromSymbol(selectedFusion.id)].filter(Boolean);
  } else {
    const seedEntry = selectedFusionSeedEntry(pool);
    entries = seedEntry ? pool.map((bodyEntry) => buildFusionSpeciesEntry(seedEntry, bodyEntry)).filter(Boolean) : [];
  }
...
async function ensureDexSpeciesDetail(speciesId, options = {}) {
  ...
  if (isFusionSymbol(normalizedId)) {
    ...
    const fusionEntry = buildFusionSpeciesEntry(resolvedHead, resolvedBody) || current;
    if (fusionEntry) {
...
async function refreshDexFusionPreview(options = {}) {
  ...
  state.fusionBuilder.preview = buildFusionSpeciesEntry(resolvedHead, resolvedBody);
...
async function cloneFusionBuilderSpecies() {
  ...
  newSpeciesDraft(preview, {
    ...
    variantFamily: `${preview.name} Variant Line`
  });
  showMessage(`Started a regional variant draft from the ${preview.name} fusion. The original fusion and both parent species stay untouched.`, "success");
}
...
function renderDexOverviewPanel(entry) {
  ...
  panel.innerHTML = `
    <div class="panel-heading">
      ...
      <p>${escapeHtml(entry.category)} · ${escapeHtml(entry.source === "base_game" ? "Base Game" : titleizeToken(entry.kind))}</p>
    </div>
...
function renderDexMovesPanel(entry) {
  ...
  if (entry.detail_level !== "full") {
    ensureDexSpeciesDetail(entry.id, { silent: true }).catch(() => {});
    panel.innerHTML = `
      <div class="panel-heading">
        <h3>Learnsets</h3>
        <p>Loading the full learnset and move compatibility for ${escapeHtml(entry.name)}.</p>
      </div>
      <p class="muted-copy">The Pokédex summary is ready. Detailed move data is loading in the background.</p>
    `;
    return;
  }
  panel.innerHTML = `
  ...
  panel.innerHTML = `
    <div class="panel-heading compact">
      <div>
        ...
        <p>Pick any installed head and body pair, preview that fusion on demand, then clone it into a new regional variant.</p>
      </div>
      <span class="summary-badge ${ready ? "soft" : "neutral"}">${ready ? "Fusion Ready" : "Pick Both Parts"}</span>
    </div>
    ...
    <div class="fusion-builder-preview ${ready ? "" : "is-empty"}">
      ...
      ` : `
        <p class="muted-copy">${escapeHtml(state.fusionBuilder.loading ? "Building the fusion preview..." : (state.fusionBuilder.error || "Use the selected Pokédex entry as the head or body, or type both IDs here directly."))}</p>
      `}
    </div>
  `;
  ...
  return `
    <article class="import-entry-card">
      ...
      ${options.pending ? `
        <div class="import-entry-actions">
          <div class="import-entry-meta">
            <span class="status-chip neutral">${escapeHtml(entry?.sender || "Pokedex Studio")}</span>
          </div>
...
function renderLivePreview() {
  ...
  const sourceLabel = entry.source === "base_game" ? "Base Game" : titleizeToken(entry.kind || "draft");
...
function buildDraftFromSource(source = null, options = {}) {
  ...
  return {
    ...
    variant_family: targetKind === "regional_variant" ? (options.variantFamily || normalized.variant_family || `${normalized.name} Variant Line`) : "",
...
async function cloneSelectedDexSpecies() {
  ...
  newSpeciesDraft(entry, {
    ...
    variantFamily: `${entry.name} Variant Line`
  });
  showMessage(`Started a regional variant draft from ${entry.name}. The original species will stay untouched.`, "success");
}
...
function computeValidation(payload) {
  ...
  if (!payload.pokedex_entry) {
    suggestions.push({ title: "Pokedex entry missing", body: "A short in-game dex entry helps the species feel like a complete official entry once it reaches the bedroom PC." });
  }
