module TravelExpansionFramework
  ...
  def build_external_manifest(project_info)
    ...
    manifest = {
      ...
      :description      => ["Linked external install at #{project_info[:root]}"],
    ...
    manifest[:travel_nodes] << {
      ...
      :name            => project_info[:display_name],
      :description     => (anchor_profile[:story_anchor] ? "Start at the linked install's normal opening flow." : "Travel into the linked install at #{project_info[:root]}"),
    ...
    if show_exploration_node && exploration_anchor && !anchors_match?(exploration_anchor, entry_anchor)
      manifest[:travel_nodes] << {
        ...
        :description     => "Skip the opening flow and enter a safer exploratory anchor inside the linked install.",
