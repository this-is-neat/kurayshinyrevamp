#===============================================================================
#  DynamicPokemonSprite and DynamicTrainerSprite classes for EBDX
#  Ported from Elite Battle DX's EBDX Sprites.rb
#===============================================================================
...
class DynamicPokemonSprite
  ...
  def initialize(doublebattle, index, viewport = nil, battle = nil)
    ...
    # Substitute bitmap - use rescue to handle missing graphics
    begin
    ...
    @scale = 2
    @fp = {}
    # loads shadow particles for battler animation - wrapped in rescue for missing graphics
    begin
      for i in 0...16
      ...
      # Skip if particle graphics missing
    end
    ...
    begin
      for i in 0...16
        ...
        @fp["d#{i}"].center! if @fp["d#{i}"].respond_to?(:center!)
        @fp["d#{i}"].opacity = 0
        @fp["d#{i}"].toggle = 1 if @fp["d#{i}"].respond_to?(:toggle=)
      end
    rescue
      # Skip if particle graphics missing
    end
  ...
  #-----------------------------------------------------------------------------
  def sideSize; return @battle.pbSideSize(@index) rescue 1; end
  ...
  def bitmap; return @bitmap ? @bitmap.bitmap.clone : nil; end
  ...
  def visible=(val)
    ...
    if @fp
      vis = val
      vis = false if @hidden || @fainted
      for key in @fp.keys
        next if !@fp[key]
        if key.is_a?(String) && key.include?("c")
          vis = false if !@charged
        else
          vis = false if !@isShadow
        end
        @fp[key].visible = vis
      end
    end
  ...
  def getCenter(zoom = true)
    ...
    x = self.x
    y = self.y + (bmp.height - self.oy)*zoom - bmp.height*zoom/2
    return x, y
  end
  #-----------------------------------------------------------------------------
  ...
  def dispose
    ...
    @substitute.dispose if @substitute && @substitute.respond_to?(:dispose)
  ...
  def setPokemonBitmap(pokemon, back = false, species = nil)
    ...
    @isShadow = true if @pokemon.respond_to?(:shadowPokemon?) && @pokemon.shadowPokemon?
  ...
  def resetParticles
    if @fp
      for key in @fp.keys
        @fp[key].visible = false if @fp[key]
      end
    end
    @isShadow = false
    @charged = false
  ...
  def shadowUpdate
    ...
    return if !@loaded
    return if self.disposed? || !@bitmap || @bitmap.disposed?
    ...
    for i in 0...16
      ...
      next if !@isShadow
      if @fp[i].opacity <= 0
        @fp[i].toggle = 2 if @fp[i].respond_to?(:toggle=)
        z = [0.5,0.6,0.7,0.8,0.9,1.0][rand(6)]
        @fp[i].param = z if @fp[i].respond_to?(:param=)
        bmp = self.bitmap
        next if !bmp
        @fp[i].x = self.x - bmp.width*self.zoom_x/2 + rand(bmp.width)*self.zoom_x
        @fp[i].y = self.y - 64*self.zoom_y + rand(64)*self.zoom_y
        @fp[i].z = (rand(2)==0) ? self.z - 1 : self.z + 1
        @fp[i].speed = (rand(2)==0) ? +1 : -1 if @fp[i].respond_to?(:speed=)
        @fp[i].src_rect.x = rand(2)*@fp[i].bitmap.width/2
      end
      @fp[i].zoom_x = (@fp[i].param rescue 1)*self.zoom_x
      @fp[i].zoom_y = (@fp[i].param rescue 1)*self.zoom_y
      # @fp[i].param -= 0.01/self.delta
      @fp[i].y -= 1
      @fp[i].opacity += 8*(@fp[i].toggle rescue 1)/self.delta
      @fp[i].toggle = -1 if @fp[i].respond_to?(:toggle=) && @fp[i].opacity >= 255
    end
  ...
  def energyUpdate
    ...
    for i in 0...16
      ...
      if @fp["d#{i}"].opacity <= 0
        ...
        @fp["d#{i}"].toggle = rand(2) == 0 ? 1 : -1 if @fp["d#{i}"].respond_to?(:toggle=)
      end
      @fp["d#{i}"].zoom_x -= 0.02/self.delta
      @fp["d#{i}"].zoom_y -= 0.02/self.delta
      @fp["d#{i}"].opacity -= 8/self.delta
      @fp["d#{i}"].angle += 2*(@fp["d#{i}"].toggle rescue 1)/self.delta
    end
...
class DynamicTrainerSprite < DynamicPokemonSprite
  ...
  #-----------------------------------------------------------------------------
  def totalFrames; @bitmap ? @bitmap.animationFrames : 1; end
  ...
  def setTrainerBitmap(file = nil)
    ...
    trainerid = file.nil? ? nil : EliteBattle.id_from_trainer_file(file) rescue nil
    # gets additional scale (if applicable)
    s = EliteBattle.get_trainer_data(trainerid, :SCALE, @trainer) rescue nil
    scale = (!s.nil? && s.is_a?(Numeric)) ? s : EliteBattle::TRAINER_SPRITE_SCALE
    sp = EliteBattle.get_trainer_data(trainerid, :SPRITESPEED, @trainer) rescue nil
    speed = (!sp.nil? && sp.is_a?(Numeric)) ? sp : 2
    # loads bitmap
    if !pbResolveBitmap(file)
      tfile = @trainer.nil? ? nil : "Graphics/Trainers/#{@trainer.trainer_type}"
      EliteBattle.log.warn("Could not find the Trainer sprite: #{file.nil? ? tfile : file}") if EliteBattle.respond_to?(:log) && EliteBattle.log.respond_to?(:warn)
    ...
    x = EliteBattle.get_trainer_data(trainerid, :X, @trainer) rescue nil
    y = EliteBattle.get_trainer_data(trainerid, :Y, @trainer) rescue nil
    a = EliteBattle.get_trainer_data(trainerid, :ALTITUDE, @trainer) rescue nil
    @sprite.ox -= x if !x.nil? && x.is_a?(Numeric)
    @sprite.oy -= y if !y.nil? && y.is_a?(Numeric)
    @sprite.oy += a if !a.nil? && a.is_a?(Numeric)
    @loaded = true
