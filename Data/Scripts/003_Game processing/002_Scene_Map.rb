  ...
  def update
    ...
    if !pbMapInterpreterRunning?
      ...
      if $game_temp.moving_furniture
        placeFurnitureMenu() if !mp_ui_blocked && !chat_typing && Input.trigger?(Input::USE)
        rotate__held_furniture_left if Input.trigger?(Input::JUMPDOWN)
        rotate_held_furniture_right if Input.trigger?(Input::JUMPUP)
      end

      if !mp_ui_blocked && !chat_typing && Input.trigger?(Input::USE)
        $PokemonTemp.hiddenMoveEventCalling = true
      elsif !mp_ui_blocked && !chat_typing && Input.trigger?(Input::BACK)
      ...
      elsif !mp_ui_blocked && !chat_typing && Input.trigger?(Input::SPECIAL)
        unless $game_system.menu_disabled || $game_player.moving?
