...
class PokemonSystem
  ...
  attr_accessor :shinyodds
  attr_accessor :unfusetraded
  attr_accessor :nomoneylost
  attr_accessor :sb_maxing
  attr_accessor :sb_soullinked
  ...
  attr_accessor :speedtoggle_mode_version
  ...
  def initialize
    ...
    @shinyodds = Settings::SHINY_POKEMON_CHANCE || 16
    @unfusetraded = 0
    @nomoneylost = 0
    @sb_maxing = 1
    @sb_soullinked = 0
    ...
    @speedvalue = 2
    @speedtoggle = 0
    @speedtoggle_mode_version = 2
    @speedvaluedef = 0
    @speeduplimit = 4
    @shiny_cache = 0
    @skipcaughtprompt = 0
    @skipcaughtnickname = 0
    @levelcapbehavior = 0
  ...
  def load_file_data(saved)
    ...
    @tutornet = saved.tutornet if saved.tutornet
  ...
  def migrate_speedtoggle_mode!
    if !@speedtoggle_mode_version
      @speedtoggle = 2 if @speedtoggle == 1
      @speedtoggle_mode_version = 2
    end
  end
end
...
class ButtonOption < Option
  ...
  def initialize(name, actionProc, description = "", label = nil)
    super(description)
    @name = name
    @actionProc = actionProc
    @values = [label || _INTL("Open")]
  end
...
class ButtonsOption < Option
  ...
  def activate
    @selectProc.call(@index) if @selectProc
  end
end
      ...
      loop do
        ...
        if Input.trigger?(Input::BACK)
          ...
          if @sprites["option"].index < @PokemonOptions.length
            selected_option = @PokemonOptions[@sprites["option"].index]
            if selected_option.respond_to?(:activate)
              selected_option.activate
            elsif selected_option.respond_to?(:trigger)
              selected_option.trigger
            end
            if selected_option.respond_to?(:activate) || selected_option.respond_to?(:trigger)
              refresh_options if respond_to?(:refresh_options)
              if respond_to?(:needs_refresh?) && needs_refresh?
                refresh_options if respond_to?(:refresh_options)
              end
              @sprites["option"].refresh
              @mustUpdateDescription = true
...
class SLOptionsScene < PokemonOption_Scene
  ...
  def createButtonsOption(index)
    if !$KURAY_OPTIONSNAME_LOADED
      ...
      if tempload.is_a?(Array) && tempload.length == 12
...
class VanillaOptSc_1 < PokemonOption_Scene
  ...
  def pbGetOptions(inloadscreen = false)
    ...
    options << EnumOption.new(_INTL("Download data"), [_INTL("On"), _INTL("Off")],
                              proc { $PokemonSystem.download_sprites },
                              proc { |value| $PokemonSystem.download_sprites = value },
  ...
  def pbGetInGameOptions
    ...
    if $game_switches
      options << EnumOption.new(_INTL("Autosave"), [_INTL("On"), _INTL("Off")],
    ...
    if $game_switches && ($game_switches[SWITCH_NEW_GAME_PLUS] || $game_switches[SWITCH_BEAT_THE_LEAGUE])
      options << EnumOption.new(_INTL("Battle type"), [_INTL("1v1"), _INTL("2v2"), _INTL("3v3"), _INTL("Random")],
                                ...
                                proc { |value|
                                  if value == 0
                                    ...
                                    $game_variables[VAR_DEFAULT_BATTLE_TYPE] = [0, 0]
                                  else
                                    $game_variables[VAR_DEFAULT_BATTLE_TYPE] = [1, 1]
                                  end
...
class KurayOptionsScene < PokemonOption_Scene
  ...
  def pbGetOptions(inloadscreen = false)
    options = []
    options << ButtonOption.new(_INTL("Shinies"), proc { openKuray1 }, "Customize shinies features")
    options << ButtonOption.new(_INTL("Battles & Pokemons"), proc { openKuray2 }, "Customize battles & pokemons features")
    options << ButtonOption.new(_INTL("Graphics"), proc { openKuray3 }, "Customize graphics features")
    options << ButtonOption.new(_INTL("Self-Battle & Import"), proc { openKuray5 }, "Self-battling & import features")
    options << ButtonOption.new(_INTL("Challenges"), proc { openKuray6 }, "Challenges")
    options << ButtonOption.new(_INTL("Others"), proc { openKuray4 }, "Customize others features")
    options << EnumOption.new(_INTL("Increment Slider by"), [_INTL("1"), _INTL("10"), _INTL("100"), _INTL("1000"), _INTL("10000")],
...
class KurayOptSc_1 < PokemonOption_Scene
  ...
  def pbGetInGameOptions
    ...
    options << SliderOption.new(_INTL("Shiny Gamble Odds"), 0, 1000, $PokemonSystem.raiser,
                                ...
                                proc { |value| $PokemonSystem.kuraygambleodds = value if $PokemonSystem.kuraygambleodds != value },
...
class KurayOptSc_2 < PokemonOption_Scene
  ...
  def pbGetInGameOptions
    options = []
    options << ButtonOption.new(_INTL("### PER-SAVE FILE ###"), proc {}, "", "")
    options << EnumOption.new(_INTL("Wild Battles"), [_INTL("1v1"), _INTL("2v2"), _INTL("3v3"), _INTL("Random")],
                              ...
                              ["Wild battles always 1v1",
                               ...
                               "Wild battles randomly pick 1v1, 2v2 or 3v3"])
    ...
    options << SliderOption.new(_INTL("    HP"), 0, 100, 5, proc { $PokemonSystem.custom_bst_sliders[:HP] }, proc { |value| $PokemonSystem.custom_bst_sliders[:HP] = value }, "Percentage of base HP contributed.")
    options << SliderOption.new(_INTL("    Attack"), 0, 100, 5, proc { $PokemonSystem.custom_bst_sliders[:ATTACK] }, proc { |value| $PokemonSystem.custom_bst_sliders[:ATTACK] = value }, "Percentage of base Attack contributed.")
    options << SliderOption.new(_INTL("    Defense"), 0, 100, 5, proc { $PokemonSystem.custom_bst_sliders[:DEFENSE] }, proc { |value| $PokemonSystem.custom_bst_sliders[:DEFENSE] = value }, "Percentage of base Defense contributed.")
    options << SliderOption.new(_INTL("    Special Attack"), 0, 100, 5, proc { $PokemonSystem.custom_bst_sliders[:SPECIAL_ATTACK] }, proc { |value| $PokemonSystem.custom_bst_sliders[:SPECIAL_ATTACK] = value }, "Percentage of base Special Attack contributed.")
    options << SliderOption.new(_INTL("    Special Defense"), 0, 100, 5, proc { $PokemonSystem.custom_bst_sliders[:SPECIAL_DEFENSE] }, proc { |value| $PokemonSystem.custom_bst_sliders[:SPECIAL_DEFENSE] = value }, "Percentage of base Special Defense contributed.")
    options << SliderOption.new(_INTL("    Speed"), 0, 100, 5, proc { $PokemonSystem.custom_bst_sliders[:SPEED] }, proc { |value| $PokemonSystem.custom_bst_sliders[:SPEED] = value }, "Percentage of base Speed contributed.")
    options << EnumOption.new(_INTL("NPC Fusion BaseStats"), [_INTL("Off"), _INTL("Head"), _INTL("Better")],
                              ...
                              ["Default NPC fusion base stats.",
                               "Sliders determine what % of each stat comes from the head pokemon.",
                               "Sliders determine what % of each stat comes from the better base stat."])
    options << SliderOption.new(_INTL("    HP"), 0, 100, 5, proc { $PokemonSystem.custom_bst_sliders_npc[:HP] }, proc { |value| $PokemonSystem.custom_bst_sliders_npc[:HP] = value }, "Percentage of base HP contributed.")
    options << SliderOption.new(_INTL("    Attack"), 0, 100, 5, proc { $PokemonSystem.custom_bst_sliders_npc[:ATTACK] }, proc { |value| $PokemonSystem.custom_bst_sliders_npc[:ATTACK] = value }, "Percentage of base Attack contributed.")
    options << SliderOption.new(_INTL("    Defense"), 0, 100, 5, proc { $PokemonSystem.custom_bst_sliders_npc[:DEFENSE] }, proc { |value| $PokemonSystem.custom_bst_sliders_npc[:DEFENSE] = value }, "Percentage of base Defense contributed.")
    options << SliderOption.new(_INTL("    Special Attack"), 0, 100, 5, proc { $PokemonSystem.custom_bst_sliders_npc[:SPECIAL_ATTACK] }, proc { |value| $PokemonSystem.custom_bst_sliders_npc[:SPECIAL_ATTACK] = value }, "Percentage of base Special Attack contributed.")
    options << SliderOption.new(_INTL("    Special Defense"), 0, 100, 5, proc { $PokemonSystem.custom_bst_sliders_npc[:SPECIAL_DEFENSE] }, proc { |value| $PokemonSystem.custom_bst_sliders_npc[:SPECIAL_DEFENSE] = value }, "Percentage of base Special Defense contributed.")
    options << SliderOption.new(_INTL("    Speed"), 0, 100, 5, proc { $PokemonSystem.custom_bst_sliders_npc[:SPEED] }, proc { |value| $PokemonSystem.custom_bst_sliders_npc[:SPEED] = value }, "Percentage of base Speed contributed.")
    options << EnumOption.new(_INTL("Dominant Fusion Types"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.dominant_fusion_types }, proc { |value| $PokemonSystem.dominant_fusion_types = value }, ["Brings back the dominant/inverse fusion types from pre-v6.", "Brings back the dominant/inverse fusion types from pre-v6."])
    options << EnumOption.new(_INTL("Improved Pokedex"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.improved_pokedex }, proc { |value| $PokemonSystem.improved_pokedex = value }, ["Don't use the Improved Pokedex", "Registers a fusions base Pokemon to the Pokedex when catching/evolving"])
    options << EnumOption.new(_INTL("Enable EvoLock"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.kuray_no_evo }, proc { |value| $PokemonSystem.kuray_no_evo = value }, ["Can't EvoLock a Pokemon without holding everstone", "Can EvoLock Pokemons in the PC"])
    options << EnumOption.new(_INTL("Unfuse Traded"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.unfusetraded }, proc { |value| $PokemonSystem.unfusetraded = value }, ["You cannot unfuse traded Pokemons.", "You can unfuse traded Pokemons."])
    options << EnumOption.new(_INTL("Recover Consumables"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.recover_consumables }, proc { |value| $PokemonSystem.recover_consumables = value }, ["Don't recover consumable items after battle", "Recover consumable items after battle"])
    options << SliderOption.new(_INTL("ExpAll Redistribution"), 0, 10, 1, proc { $PokemonSystem.expall_redist }, proc { |value| $PokemonSystem.expall_redist = value }, "0 = Off, 10 = Max | Redistributes total exp from expAll to lower level pokemon")
    options << EnumOption.new(_INTL("Auto-Battle"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.autobattler }, proc { |value| $PokemonSystem.autobattler = value }, ["You fight your own battles", "Allows Trapstarr to take control of your pokemon"])
    options << EnumOption.new(_INTL("Auto-Battle Shortcut"), [_INTL("On"), _INTL("Off")], proc { $PokemonSystem.autobattleshortcut }, proc { |value| $PokemonSystem.autobattleshortcut = value }, ["Allows to toggle Auto-Battle in battles with the X key.", "Disable the Auto-Battle toggle shortcut."])
    options << EnumOption.new(_INTL("Auto-Battle Shiny Stop"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.autobattlershiny }, proc { |value| $PokemonSystem.autobattlershiny = value }, ["Do NOT stop Auto-Battle if a shiny enemy is detected.", "Automatically stops Auto-Battle if a shiny enemy is detected."])
    options << EnumOption.new(_INTL("Damage Variance"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.damage_variance }, proc { |value| $PokemonSystem.damage_variance = value }, ["Damage Variance is disabled.", "Damage Variance is enabled."])
    options << EnumOption.new(_INTL("Head Legendary Breeding"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.legendarybreed }, proc { |value| $PokemonSystem.legendarybreed = value }, ["Legendary Head cannot breed (like new PIF).", "Legendary Head can breed (like old PIF)."])
    options << EnumOption.new(_INTL("Event Moves"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.eventmoves }, proc { |value| $PokemonSystem.eventmoves = value }, "Event Moves are available at the Battle Factory Egg Move Tutor.")
    options << EnumOption.new(_INTL("Modern Hail"), [_INTL("Off"), _INTL("Hail"), _INTL("Snow")], proc { $PokemonSystem.modernhail }, proc { |value| $PokemonSystem.modernhail = value }, ["Vanilla hail.", "Ice types receive a defensive boost during hail.", "Hail becomes Snow, behaves like Gen 9+."])
    options << EnumOption.new(_INTL("Frostbite"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.frostbite }, proc { |value| $PokemonSystem.frostbite = value }, ["Vanilla Frozen Status.", "Frostbite Status from Gen 9+ replaces Frozen Status."])
    options << EnumOption.new(_INTL("Drowsy"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.drowsy }, proc { |value| $PokemonSystem.drowsy = value }, ["Vanilla Sleep Status.", "Drowsy Status from Gen 9+ replaces Sleep Status."])
    options << EnumOption.new(_INTL("Bug Type Buffs"), [_INTL("Off"), _INTL("Defensive"), _INTL("+ Offensive")], proc { $PokemonSystem.bugbuff }, proc { |value| $PokemonSystem.bugbuff = value }, ["Vanilla Bug Types.", "Bug Types resist Fairy, Psychic, and Dark.", "Fairy Types are weak to Bug Type Moves."])
    options << EnumOption.new(_INTL("Ice Type Buffs"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.icebuff }, proc { |value| $PokemonSystem.icebuff = value }, ["Vanilla Ice Types.", "Ice Types now resist Water and Flying."])
    options << EnumOption.new(_INTL("Skip Caught Prompt"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.skipcaughtprompt }, proc { |value| $PokemonSystem.skipcaughtprompt = value }, ["If your party is full, allows you to put the caught pokemon on your party.", "If your party is full, always send the caught pokemon to the PC."])
    options << EnumOption.new(_INTL("Skip Caught Nickname"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.skipcaughtnickname }, proc { |value| $PokemonSystem.skipcaughtnickname = value }, ["Prompts you to nickname newly caught pokemon.", "Never prompts to nickname newly caught pokemon."])
    options << EnumOption.new(_INTL("Show Lv. in BSM"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.showlevel_nolevelmode }, proc { |value| $PokemonSystem.showlevel_nolevelmode = value }, ["Base Stats Mode hides the level of Pokemons in battle.", "Base Stats Mode shows the level of Pokemons (level don't reflect stats!)."])
    options << EnumOption.new(_INTL("No-EVs Mode"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.noevsmode }, proc { |value| $PokemonSystem.noevsmode = value }, ["Pokemon EVs exist.", "Pokemon EVs are disabled."])
    options << EnumOption.new(_INTL("Max IVs Mode"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.maxivsmode }, proc { |value| $PokemonSystem.maxivsmode = value }, ["Disabled.", "Pokemon IVs are always at max."])
    options << EnumOption.new(_INTL("EVs Train Mode"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.evstrain }, proc { |value| $PokemonSystem.evstrain = value }, ["EVs yielding works as expected.", "Enemies do not yield EVs (except yielding from held Power-items)."])
    options << EnumOption.new(_INTL("Rocket Mode"), [_INTL("Off"), _INTL("On"), _INTL("All Balls")], proc { $PokemonSystem.rocketballsteal }, proc { |value| $PokemonSystem.rocketballsteal = value }, ["Rocket Balls don't steal Pokemons.", "Rocket Balls can steal Pokemons.", "Every Balls can steal Pokemons."])
    options << SliderOption.new(_INTL("Trainer Exp. Boost"), 0, 1000, 50, proc { $PokemonSystem.trainerexpboost }, proc { |value| $PokemonSystem.trainerexpboost = value }, "+x% | Boosts exp. gained in trainer battles (Default: +50%)")
    options << EnumOption.new(_INTL("K-Eggs Fusion Pool"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.kurayeggs_fusionpool }, proc { |value| $PokemonSystem.kurayeggs_fusionpool = value }, ["In case of fusion, the other Pokemon is random.", "In case of fusion, the other Pokemon must be from k-egg's pool as well."])
    options << EnumOption.new(_INTL("K-Eggs Rarity"), [_INTL("On"), _INTL("Off")], proc { $PokemonSystem.kurayeggs_rarity }, proc { |value| $PokemonSystem.kurayeggs_rarity = value }, ["Pokemons' rarity in k-eggs depend of their catch rate.", "All Pokemons have the same odds to come from the k-eggs."])
    options << SliderOption.new(_INTL("K-Eggs Fusion Odds"), 0, 100, 1, proc { $PokemonSystem.kurayeggs_fusionodds }, proc { |value| $PokemonSystem.kurayeggs_fusionodds = value }, "x% | Probability that a fusion hatches from Kuray Eggs (Default: 20%)")
    options << EnumOption.new(_INTL("K-Eggs Usage"), [_INTL("Pokemon"), _INTL("Egg")], proc { $PokemonSystem.kurayeggs_instanthatch }, proc { |value| $PokemonSystem.kurayeggs_instanthatch = value }, ["When Kuray Egg item used, spawns a Pokemon.", "When Kuray Egg item used, spawns an Egg that contains a Pokemon."])
    options << SliderOption.new(_INTL("K-Eggs Rewards"), 0, 10, 1, proc { $PokemonSystem.gymrewardeggs }, proc { |value| $PokemonSystem.gymrewardeggs = value }, "Numbers of obtained K-Eggs upon unlocking each one through progression (Default: 3)")
    return options
  end
end
...
class KurayOptSc_3 < PokemonOption_Scene
  ...
  def pbGetOptions(inloadscreen = false)
    options = []
    options << ButtonOption.new(_INTL("### GLOBAL ###"), proc {}, "", "")
    options << EnumOption.new(_INTL("Big Pokémon Icons"), [_INTL("Off"), _INTL("Limited"), _INTL("All")], proc { $PokemonSystem.kuraybigicons }, proc { |value| $PokemonSystem.kuraybigicons = value }, ["Pokémon will use their small box sprites for icons", "Pokémon icons will use their full-size battle sprites (except in boxes)", "Pokémon icons will use their full-size battle sprites"])
    options << EnumOption.new(_INTL("Fusion Preview"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.kurayfusepreview }, proc { |value| $PokemonSystem.kurayfusepreview = value }, ["Don't preview what unknown fusions look like", "Preview what unknown fusions look like"])
    options << EnumOption.new(_INTL("Type Display"), [_INTL("Off"), _INTL("Ico"), _INTL("TCG"), _INTL("Sqr"), _INTL("FGM"), _INTL("Txt")], proc { $PokemonSystem.typedisplay }, proc { |value| $PokemonSystem.typedisplay = value }, ["Don't draw the type indicator in battle", "Draws handmade custom type icons in battle | By Lolpy1", "Draws TCG themed type icons in battle", "Draws the square type icons in battle | Triple Fusion by Lolpy1", "Draws handmade custom type icons in battle | By FairyGodmother", "Draws the text type display in battle"])
    options << EnumOption.new(_INTL("Swap BattleGUI"), [_INTL("Off"), _INTL("Type 1"), _INTL("Type 2")], proc { $PokemonSystem.battlegui }, proc { |value| $PokemonSystem.battlegui = value }, ["This feature is a work in progress, more to come soon", "Swaps the HP/Exp bar to v1 | created by Mirasein", "Swaps the HP/Exp bar to v2 | created by Mirasein"])
    options << EnumOption.new(_INTL("Dark Mode"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.darkmode }, proc { |value| $PokemonSystem.darkmode = value }, ["Default UI", "Swaps the message graphics during battle"])
...
class KurayOptSc_6 < PokemonOption_Scene
  ...
  def pbGetOptions(inloadscreen = false)
    ...
    if $scene && $scene.is_a?(Scene_Map)
      ...
      options << ButtonOption.new(_INTL("### EMPTY ###"), proc {}, "", "")
    end
    return options
  end
  ...
  def pbGetInGameOptions
    options = []
    options << ButtonOption.new(_INTL("### PER-SAVE FILE ###"), proc {}, "", "")
    options << EnumOption.new(_INTL("Metronome Madness"), [_INTL("Off"), _INTL("Normal"), _INTL("Hard")], proc { $PokemonSystem.ch_metronome }, proc { |value| $PokemonSystem.ch_metronome = value }, ["Metronome disabled.", "All Pokemons are forced to use Metronome. [Normal]", "Only your Pokemons are forced to use Metronome. [Hard]"])
    options << EnumOption.new(_INTL("Letdown"), [_INTL("Off"), _INTL("1%"), _INTL("5%"), _INTL("10%"), _INTL("25%"), _INTL("50%")], proc { $PokemonSystem.ch_letdown }, proc { |value| $PokemonSystem.ch_letdown = value }, ["Letdown disabled.", "1% chance that Pokemons use Splash instead.", "5% chance that Pokemons use Splash instead.", "10% chance that Pokemons use Splash instead.", "25% chance that Pokemons use Splash instead.", "50% chance that Pokemons use Splash instead."])
    options << EnumOption.new(_INTL("Letdown Player Only"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.ch_letdownplayer }, proc { |value| $PokemonSystem.ch_letdownplayer = value }, ["Letdown doesn't only affect the player.", "Letdown only affects the player."])
    options << EnumOption.new(_INTL("Berserker"), [_INTL("Off"), _INTL("Easy"), _INTL("Normal"), _INTL("Hard"), _INTL("Chaos")], proc { $PokemonSystem.ch_berserker }, proc { |value| $PokemonSystem.ch_berserker = value }, ["Berserker disabled.", "Opponent stats raise by 1 every 3 turns. [Easy]", "Opponent stats raise by 1 every 2 turns. [Normal]", "Opponent stats raise by 1 every turn. [Hard]", "Opponent stats raise by 2 every turn. [Hardcore]"])
    return options
  end
end
...
class KurayOptSc_4 < PokemonOption_Scene
  ...
  def pbGetOptions(inloadscreen = false)
    options = []
    options << ButtonOption.new(_INTL("### GLOBAL ###"), proc {}, "", "")
    options << EnumOption.new(_INTL("Debug Exclusive"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.debugfeature }, proc { |value| $PokemonSystem.debugfeature = value }, ["Some debug features are available without debug (magic boots)", "All debug features requires debug (magic boots)"])
    options << EnumOption.new(_INTL("Quicksave with S"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.quicksave }, proc { |value| $PokemonSystem.quicksave = value }, "Quicksave with S")
    options << EnumOption.new(_INTL("Speed-up Type"), [_INTL("Toggle"), _INTL("Set"), _INTL("Hold")],
                              proc { $PokemonSystem.speedtoggle },
                              proc { |value| $PokemonSystem.speedtoggle = value },
                              ["Button cycles through speed levels",
                               "Button switches between the default and speed-up hold speeds",
                               "Hold the button to speed-up the game"])
    options << SliderOption.new(_INTL("Default Speed (Set/Hold)"), 1, 10, 1, proc { $PokemonSystem.speedvaluedef }, proc { |value| $PokemonSystem.speedvaluedef = value }, "Sets the default speed when the speed-up button is not active (Default: 1x)")
    options << SliderOption.new(_INTL("Speed-up Speed (Set/Hold)"), 1, 10, 1, proc { $PokemonSystem.speedvalue }, proc { |value| $PokemonSystem.speedvalue = value }, "Sets the speed used by Set/Hold mode when the speed-up button is active (Default: 2x)")
    options << SliderOption.new(_INTL("Speed-up Limit (Toggle)"), 1, 10, 1, proc { $PokemonSystem.speeduplimit }, proc { |value| $PokemonSystem.speeduplimit = value }, "Sets the limit of speed-up when using toggle mode (Default: 5x)")
    if $scene && $scene.is_a?(Scene_Map)
      options.concat(pbGetInGameOptions)
    end
    return options
  ...
  def pbGetInGameOptions
    options = []
    options << ButtonOption.new(_INTL("### PER-SAVE FILE ###"), proc {}, "", "")
    options << EnumOption.new(_INTL("Kuray QoL"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.kurayqol }, proc { |value| $PokemonSystem.kurayqol = value }, ["Kuray's QoL features OFF", "Kuray's QoL features ON"])
    options << EnumOption.new(_INTL("Tutor.net"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.tutornet }, proc { |value| $PokemonSystem.tutornet = value }, ["Save Tutors/TMs in a handy list: OFF", "Save Tutors/TMs in a handy list: ON"])
    options << EnumOption.new(_INTL("Kuray's Shenanigans"), [_INTL("On"), _INTL("Off")], proc { $PokemonSystem.shenanigans }, proc { |value| $PokemonSystem.shenanigans = value }, ["You're playing with Shenanigans! (Easter Eggs)", "You're playing normally! (No Easter Eggs)"])
    ...
    options << EnumOption.new(_INTL("Overworld Poison"), [_INTL("Off"), _INTL("On"), _INTL("On+Healing")], proc { $PokemonSystem.walkingpoison }, proc { |value| $PokemonSystem.walkingpoison = value }, ["Everyone takes dmg on overworld poison.", "Some abilities immune to dmg on overworld poison.", "Some abilities heal instead of taking dmg on overworld poison."])
    options << EnumOption.new(_INTL("PokeRadar+"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.pokeradarplus }, proc { |value| $PokemonSystem.pokeradarplus = value }, "Adds a chain count display. Chains won't randomly drop.")
    return options
  end
end
...
class KurayOptSc_5 < PokemonOption_Scene
  ...
  def pbGetInGameOptions
    options = []
    options << ButtonOption.new(_INTL("### PER-SAVE FILE ###"), proc {}, "", "")
    options << EnumOption.new(_INTL("Battle Size"), [_INTL("1"), _INTL("2"), _INTL("3"), _INTL("4"), _INTL("5"), _INTL("6")], proc { $PokemonSystem.sb_battlesize }, proc { |value| $PokemonSystem.sb_battlesize = value }, ["1 enemy Pokemon", "2 enemy Pokemons", "3 enemy Pokemons", "4 enemy Pokemons", "5 enemy Pokemons", "6 enemy Pokemons"])
    options << EnumOption.new(_INTL("Player Size"), [_INTL("1"), _INTL("2"), _INTL("3"), _INTL("4"), _INTL("5"), _INTL("6")], proc { $PokemonSystem.sb_randomizesize }, proc { |value| $PokemonSystem.sb_randomizesize = value }, ["1 player Pokemon", "2 player Pokemons", "3 player Pokemons", "4 player Pokemons", "5 player Pokemons", "6 player Pokemons"])
    options << EnumOption.new(_INTL("Level"), [_INTL("Default"), _INTL("1"), _INTL("5"), _INTL("10"), _INTL("50"), _INTL("70"), _INTL("100")], proc { $PokemonSystem.sb_level }, proc { |value| $PokemonSystem.sb_level = value }, ["Pokemons keep their original level", "Pokemons are level 1", "Pokemons are level 5", "Pokemons are level 10", "Pokemons are level 50", "Pokemons are level 70", "Pokemons are level 100"])
    options << EnumOption.new(_INTL("Randomize Team"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.sb_randomizeteam }, proc { |value| $PokemonSystem.sb_randomizeteam = value }, ["Doesn't randomize your player team", "Randomize your player team as well"])
    options << EnumOption.new(_INTL("Randomize Share"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.sb_randomizeshare }, proc { |value| $PokemonSystem.sb_randomizeshare = value }, ["Doesn't allow randomize to make you share the same Pokemons", "Randomize might give you and the enemy the same Pokemons"])
    options << EnumOption.new(_INTL("Players Folder"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.sb_playerfolder }, proc { |value| $PokemonSystem.sb_playerfolder = value }, ["Does not use the Players folder to randomize the player's team.", "Uses the Players Folder to randomize the player's team."])
    options << EnumOption.new(_INTL("Team Select"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.sb_select }, proc { |value| $PokemonSystem.sb_select = value }, ["Doesn't prompt you to select your team", "Let you select your team"])
    options << EnumOption.new(_INTL("Limitless Select"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.sb_maxing }, proc { |value| $PokemonSystem.sb_maxing = value }, ["You may only use the same party size", "You may perform 6v1, etc (bypass the party size)"])
    options << EnumOption.new(_INTL("Stat Tracker"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.sb_stat_tracker }, proc { |value| $PokemonSystem.sb_stat_tracker = value }, ["Does not display the stat tracker during AutoBattle + Battle Loop", "Shows stats such as Win/Loss tracker for Self-battle/Auto-battle"])
    options << EnumOption.new(_INTL("Import Level"), [_INTL("Default"), _INTL("1"), _INTL("5"), _INTL("50"), _INTL("100")], proc { $PokemonSystem.importlvl }, proc { |value| $PokemonSystem.importlvl = value }, ["Imported Pokemons keep their original level.", "Imported Pokemons are set to level 1.", "Imported Pokemons are set to level 5.", "Imported Pokemons are set to level 50.", "Imported Pokemons are set to level 100."])
    options << EnumOption.new(_INTL("Import De-Evolve"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.importdevolve }, proc { |value| $PokemonSystem.importdevolve = value }, ["Imported Pokemons remain intact.", "Imported Pokemons are de-evolved into babies."])
    options << EnumOption.new(_INTL("Import as Egg"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.importegg }, proc { |value| $PokemonSystem.importegg = value }, ["Imported Pokemons remain intact.", "Imported Pokemons are turned into eggs."])
    options << EnumOption.new(_INTL("Import Without Deletion"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.importnodelete }, proc { |value| $PokemonSystem.importnodelete = value }, ["Imported Pokemons will have their .json deleted.", "Imported Pokemons will remain as .json in Import folder."])
    options << EnumOption.new(_INTL("Delete on Export"), [_INTL("Off"), _INTL("On")], proc { $PokemonSystem.exportdelete }, proc { |value| $PokemonSystem.exportdelete = value }, ["Exported Pokemons will not be deleted.", "Exported Pokemons will be deleted."])
    options << EnumOption.new(_INTL("Export Sprite on Export"), [_INTL("On"), _INTL("Off"), _INTL("Shiny")], proc { $PokemonSystem.nopngexport }, proc { |value| $PokemonSystem.nopngexport = value }, ["The .png of the Pokemon will also be exported.", "The .png (appearence) of the Pokemon will not be exported.", ".png exported as shiny, shiny appearence but pokemon un-shinified."])
    options << EnumOption.new(_INTL("Import Sprite on Import"), [_INTL("On"), _INTL("Read-Only"), _INTL("Off")], proc { $PokemonSystem.nopngimport }, proc { |value| $PokemonSystem.nopngimport = value }, ["The .png of the Pokemon will also be imported.", "The .png of the Pokemon will be read from the folder but not imported.", "The .png (appearence) of the Pokemon will not be imported."])
    return options
  end
end

class PokemonGameOption_Scene < VanillaOptSc_1
end
